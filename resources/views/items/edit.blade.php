...
<div class="flex items-center">
    <a href="{{ route('items.show', $item) }}" class="mr-4 text-gray-400 hover:text-blue-500 transition-colors duration-150">
        <i class="fas fa-arrow-left"></i>
    </a>
    <i class="fas fa-box text-blue-500 mr-2"></i>
    <span class="mr-2">Products</span>
    <i class="fas fa-chevron-right text-xs text-gray-600 mx-2"></i>
    <span class="text-gray-400 truncate">Edit "{{ $item->item_name }}"</span>
</div>
@endsection
...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
            <div class="p-6">
                <h1 class="text-2xl font-semibold text-white mb-6">Edit Product</h1>
                ...
                <form action="{{ route('items.update', $item) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input id="price" type="number" step="0.01" min="0" name="price" value="{{ old('price', $item->price) }}" 
                        ...
                        <div>
                            ...
                            <input id="quantity" type="number" min="0" name="quantity" value="{{ old('quantity', $item->inventory ? $item->inventory->quantity : 0) }}" 
                                   class="bg-gray-700 border border-gray-600 text-white rounded-md w-full p-2.5 focus:ring-blue-500 focus:border-blue-500">
                            @error('quantity')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <div class="bg-gray-700 border border-gray-600 rounded-md px-3 py-2 h-[42px] flex items-center">
                                <button type="button" id="categories-button" class="text-white hover:text-blue-500 focus:outline-none flex items-center justify-between w-full">
                                    <span id="categories-selected">{{ $item->groups->count() }} categories selected</span>
                                ...
                                <div class="p-2 space-y-1">
                                    ...
                                    <div class="flex items-center p-2 hover:bg-gray-600 rounded">
                                        <input type="checkbox" name="groups[]" id="group-{{ $group->group_id }}" value="{{ $group->group_id }}"
                                               ...
                                               {{ in_array($group->group_id, old('groups', $item->groups->pluck('group_id')->toArray())) ? 'checked' : '' }}>
                    ...
                    <div>
                        ...
                        <textarea id="item_description" name="item_description" rows="5" 
                                  class="bg-gray-700 border border-gray-600 text-white rounded-md w-full p-2.5 focus:ring-blue-500 focus:border-blue-500">{{ old('item_description', $item->item_description) }}</textarea>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-400 mb-2">Current Images</label>
                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                            @foreach($item->images as $image)
                            <div class="relative bg-gray-700 rounded overflow-hidden h-32">
                                <img src="{{ asset('storage/' . $image->image_path) }}" alt="{{ $item->item_name }}" class="w-full h-full object-contain">
                                <div class="absolute top-2 right-2">
                                    <input type="checkbox" name="delete_images[]" id="delete-{{ $image->image_id }}" value="{{ $image->image_id }}" class="hidden">
                                    <label for="delete-{{ $image->image_id }}" class="bg-red-600 text-white p-1 rounded cursor-pointer hover:bg-red-700 transition-colors duration-150">
                                        <i class="fas fa-trash-alt"></i>
                                    </label>
                                </div>
                                @if($image->is_primary)
                                <div class="absolute bottom-0 left-0 right-0 bg-blue-600 text-white text-xs py-1 px-2 text-center">
                                    Primary Image
                                </div>
                                @endif
                            </div>
                        ...
                        <p class="text-sm text-gray-400 mt-2">Check the trash icon to remove an image.</p>
                    </div>
                    @endif
                    ...
                    <div class="flex justify-end space-x-3">
                        <a href="{{ route('items.show', $item) }}" class="px-4 py-2 bg-gray-700 text-white rounded-md hover:bg-gray-600">
                            Cancel
                        </a>
...
<script>
    ...
    deleteCheckboxes.forEach(checkbox => {
        ...
        checkbox.addEventListener('change', function() {
            if (this.checked) {
                label.classList.replace('bg-red-600', 'bg-green-600');
                label.classList.replace('hover:bg-red-700', 'hover:bg-green-700');
                label.querySelector('i').classList.replace('fa-trash-alt', 'fa-check');
                this.closest('.relative').classList.add('opacity-50');
            } else {
                label.classList.replace('bg-green-600', 'bg-red-600');
                label.classList.replace('hover:bg-green-700', 'hover:bg-red-700');
                label.querySelector('i').classList.replace('fa-check', 'fa-trash-alt');
                this.closest('.relative').classList.remove('opacity-50');
            }
        });
    });
