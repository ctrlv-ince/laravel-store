        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('products.update', $product->item_id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input type="text" name="item_name" id="item_name" value="{{ old('item_name', $product->item_name) }}" required
                        ...
                        <div>
                            ...
                            <textarea name="item_description" id="item_description" rows="4" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('item_description', $product->item_description) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="group_id" id="group_id" required
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->group_id }}" {{ (old('group_id', $product->group_id) == $category->group_id) ? 'selected' : '' }}>
                                        {{ $category->group_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            @if($product->image_path)
                                <div class="mt-2 mb-4">
                                    <img src="{{ Storage::url($product->image_path) }}" alt="{{ $product->item_name }}" class="h-32 w-32 object-cover rounded-lg">
                                </div>
                            @endif
                            <input type="file" name="image" id="image" accept="image/*"
                                class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                            <p class="mt-1 text-sm text-gray-500">
                                Upload a new product image (JPEG, PNG, JPG, GIF up to 2MB) or leave empty to keep the current image
                            </p>
                        </div>
